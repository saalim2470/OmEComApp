...
const SearchResultScreen = ({ navigation }) => {
  ...
  const searchLoading = useSelector((state) => state.searchContent.isLoading);
  const [searchTxt, setSearchTxt] = useState("");
  const [searchData, setSearchData] = useState([]);
  const [isDataFound, setIsDataFound] = useState(false);
  useEffect(() => {
    if (searchResultRes?.Success) {
      searchResultRes?.Data?.length <= 0
        ? setIsDataFound(false)
        : setIsDataFound(true);
      setSearchData(searchResultRes?.Data);
    }
  }, [searchResultRes]);
  ...
  const imageListData = [
    ...
    },
    {
      ...
      assets: [
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8tclUoWIgecufJPmv-b0Pe1hvshZQtNLpYbmPnwaXqixsfoiz",
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTAcNrbQjQ9k7Jl5LhjCsNiR2SMhrKr7BLu-_uSt1Oc_nkeU1vZ",
      ],
    },
    {
      id: 3,
    ...
    <View style={commonStyle.container}>
      ...
      >
        ...
        {searchLoading ? (
          ...
          <Text
            style={{ fontFamily: "Montserrat-Medium", alignSelf: "center" }}
          >
            No Data Found
          </Text>
        )}
